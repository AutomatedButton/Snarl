...
class Level {
    ...
    makeGivenLevel(level) {
        ...
        for (var i = 0; i < rooms.length; i++) {
            this.makeRoom(rooms[i].bounds.rows, rooms[i].bounds.columns, { row: rooms[i].origin[0], col: rooms[i].origin[1] }, rooms[i]);
        }
    ...
    makeLevel() {
        ...
        this.makeRoom(5, 5, { row: 2, col: 2 });
        this.makeRoom(5, 5, { row: 8, col: 8 });
        this.makeHallway({ row: 4, col: 6 }, { row: 4, col: 10 });
        this.makeHallway({ row: 4, col: 10 }, { row: 8, col: 10 });
        // this.makeAllRooms(2);
    ...
    makeAllRooms(numRooms) {
        ...
        for (var i = 0; i < numRooms; i++) {
            ...
            var row =
                this.generateRandom(0, zoneHeight - roomHeight) +
                Math.floor(this.levelSize / rowsOfRooms) * Math.floor((rng - 1) / rowsOfRooms);
            var position = {
                row: row,
                col: col,
            };
     ...
     */
    makeRoom(givenRows, givenColumns, givenLocation, givenRoom) {
        // Buffer of where the x location starts of the room
        var rowBuffer = givenLocation.row;
        // Buffer of where the y location starts of the room
        var colBuffer = givenLocation.col;
        // Array of the room
        var room = [];
    ...
    makeAllHallways() {
        ...
        for (var i = 0; i < this.rooms.length - 1; i++) {
            ...
            // Randomly picks an y (row) and a random outside room's x (column) from the next room
            var randRow = this.generateRandom(0, room1.length);
            var randCol = this.generateRandom(0, room1[randRow].length);
            // Creates an array with both of these positions
            var location1 = { row: room1[randRow][randCol].location.row, col: room1[randRow][randCol].location.col };
            var room2 = this.rooms[i + 1];
            // Randomly picks an x and a random outside room's y from the next room
            randRow = this.generateRandom(0, room2.length);
            randCol = this.generateRandom(0, room2[randRow].length);
            // Creates an array with both of these positions
            var location2 = { row: room2[randRow][randCol].location.row, col: room2[randRow][randCol].location.col };
    ...
    createPlayer(playerName) {
        ...
        validLoc: while (!valid) {
            ...
            for (let value of Object.values(this.objects)) {
                if (value.location.row === randLoc.row && randLoc.col === value.location.col) {
                    continue validLoc;
                }
            }
            for (let value of Object.values(this.adversaries)) {
                if (value.location.row === randLoc.row && randLoc.col === value.location.col) {
                    continue validLoc;
                }
            }
     ...
     */
    createAdversaries(ghostsNum, zombNum) {
    ...
    createExitDoor() {
        ...
        var gameLevelPos = { row: exitRoom[randRow][randCol].location.row, col: exitRoom[randRow][randCol].location.col };
    ...
    createKey() {
        ...
        var gameLevelPos = { row: keyRoom[randRow][randCol].location.row, col: keyRoom[randRow][randCol].location.col };
