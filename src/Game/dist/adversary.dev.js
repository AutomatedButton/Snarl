...
/*#__PURE__*/
function () {
  function Adversary(id, type, location, layout, objects, rooms) {
  ...
  _createClass(Adversary, [{
    ...
    value: function updatePlayerLocations(playerLocations) {
    ...
    value: function moveToWall() {
      ...
      while (q.length !== 0) {
        ...
        for (var i = 0; i < 8; i++) {
          ...
          if (rr > this.layout[this.layout.length - 1][0].location.row || cc > this.layout[0][this.layout[0].length - 1].location.col) {
            continue;
          }
    ...
    key: "randomValidMove",
    value: function randomValidMove(room) {
      var dr = [-1, 1, 0, 0, 1, -1, -1, 1];
      var dc = [0, 0, 1, -1, 1, -1, 1, -1];
      var valid = false;
    ...
    key: "getShortestPath",
    value: function getShortestPath(player, room) {
      var q = Array();
      var src = new Node(this.location, null);
      q.push(src);
      var dr = [-1, 1, 0, 0, 1, -1, -1, 1];
    ...
    key: "getAdversaryRoom",
    value: function getAdversaryRoom() {
      var room = null;
      ...
      rooms: for (var i = 0; i < this.rooms.length; i++) {
        ...
        if (this.location.row >= origin.row && this.location.row < origin.row + rows && this.location.col >= origin.col && this.location.col < origin.col + cols) {
          room = this.rooms[i];
          break rooms;
        }
      }
    ...
    key: "getAdversaryHallway",
    value: function getAdversaryHallway() {
    ...
    key: "getClosestPlayer",
    value: function getClosestPlayer(layout) {
      var players = [];
      var origin = layout[0][0].location;
      var rows = layout.length;
      var cols = layout[0].length;

      for (var _i = 0, _Object$values = Object.values(this.playerLocations); _i < _Object$values.length; _i++) {
        var value = _Object$values[_i];

        if (value.location.row >= origin.row && value.location.row < origin.row + rows && value.location.col >= origin.col && value.location.col < origin.col + cols && !value.exited) {
          players.push(value);
        }
      }
