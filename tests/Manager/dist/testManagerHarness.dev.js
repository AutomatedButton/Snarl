...
function getNearbyActors(player, playerList, adversaryList) {
  ...
  for (var i = 0; i < playerList.length; i++) {
    if (Math.abs(player.position[0] - playerList[i].position[0]) <= 2 && Math.abs(player.position[1] - playerList[i].position[1]) <= 2 && player.name !== playerList[i].name) {
      nearbyActors.push(playerList[i]);
    }
  }

  for (var i = 0; i < adversaryList.length; i++) {
    if (Math.abs(player.position[0] - adversaryList[i].position[0]) <= 2 && Math.abs(player.position[1] - adversaryList[i].position[1]) <= 2) {
...
function getNearbyObjects(player, state) {
  ...
  for (var i = 0; i < state.level.objects.length; i++) {
    if (Math.abs(player.position[0] - state.level.objects[i].position[0]) <= 2 && Math.abs(player.position[1] - state.level.objects[i].position[1]) <= 2) {
